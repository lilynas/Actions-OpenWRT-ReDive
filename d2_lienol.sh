...
# 自定义定制选项
sed -i 's#192.168.123.1#10.0.0.1#g' package/base-files/files/bin/config_generate #定制默认IP
...
# 常用LuCI插件(禁用):
cat >> .config <<EOF
CONFIG_PACKAGE_smartdns=y #smartdnsDNS服务
...
CONFIG_PACKAGE_luci-app-filebrowser=y #File Browser私有云
